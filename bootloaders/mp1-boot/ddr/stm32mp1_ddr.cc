...
#define DDRCTL_REG(x, y)                                                                                               \
	{                                                                                                                  \
#x, offsetof(struct stm32mp1_ddrctl, x), offsetof(struct y, x)                                                 \
	}

#define DDRPHY_REG(x, y)                                                                                               \
	{                                                                                                                  \
#x, offsetof(struct stm32mp1_ddrphy, x), offsetof(struct y, x)                                                 \
	}

#define DDR_REG_DYN(x)                                                                                                 \
	{                                                                                                                  \
#x, offsetof(struct stm32mp1_ddrctl, x), INVALID_OFFSET                                                        \
	}

#define DDRPHY_REG_DYN(x)                                                                                              \
	{                                                                                                                  \
#x, offsetof(struct stm32mp1_ddrphy, x), INVALID_OFFSET                                                        \
	}
...
static const struct reg_desc ddr_reg[DDRCTL_REG_REG_SIZE] = {
	DDRCTL_REG_REG(mstr),	   DDRCTL_REG_REG(mrctrl0),	 DDRCTL_REG_REG(mrctrl1),	 DDRCTL_REG_REG(derateen),
	DDRCTL_REG_REG(derateint), DDRCTL_REG_REG(pwrctl),	 DDRCTL_REG_REG(pwrtmg),	 DDRCTL_REG_REG(hwlpctl),
	DDRCTL_REG_REG(rfshctl0),  DDRCTL_REG_REG(rfshctl3), DDRCTL_REG_REG(crcparctl0), DDRCTL_REG_REG(zqctl0),
	DDRCTL_REG_REG(dfitmg0),   DDRCTL_REG_REG(dfitmg1),	 DDRCTL_REG_REG(dfilpcfg0),	 DDRCTL_REG_REG(dfiupd0),
	DDRCTL_REG_REG(dfiupd1),   DDRCTL_REG_REG(dfiupd2),	 DDRCTL_REG_REG(dfiphymstr), DDRCTL_REG_REG(odtmap),
	DDRCTL_REG_REG(dbg0),	   DDRCTL_REG_REG(dbg1),	 DDRCTL_REG_REG(dbgcmd),	 DDRCTL_REG_REG(poisoncfg),
...
enum base_type { DDR_BASE, DDRPHY_BASE, NONE_BASE };
...
const struct ddr_reg_info ddr_registers[REG_TYPE_NB] = {
	[REG_REG] = {"static", ddr_reg, DDRCTL_REG_REG_SIZE, DDR_BASE},
	[REG_TIMING] = {"timing", ddr_timing, DDRCTL_REG_TIMING_SIZE, DDR_BASE},
	[REG_PERF] = {"perf", ddr_perf, DDRCTL_REG_PERF_SIZE, DDR_BASE},
	[REG_MAP] = {"map", ddr_map, DDRCTL_REG_MAP_SIZE, DDR_BASE},
	[REGPHY_REG] = {"static", ddrphy_reg, DDRPHY_REG_REG_SIZE, DDRPHY_BASE},
	[REGPHY_TIMING] = {"timing", ddrphy_timing, DDRPHY_REG_TIMING_SIZE, DDRPHY_BASE},
	[REGPHY_CAL] = {"cal", ddrphy_cal, DDRPHY_REG_CAL_SIZE, DDRPHY_BASE},
#ifdef CONFIG_STM32MP1_DDR_INTERACTIVE
	[REG_DYN] = {"dyn", ddr_dyn, DDR_REG_DYN_SIZE, DDR_BASE},
	[REGPHY_DYN] = {"dyn", ddrphy_dyn, DDRPHY_REG_DYN_SIZE, DDRPHY_BASE},
#endif
...
static void set_reg(const struct ddr_info *priv, enum reg_type type, const void *param)
{
	unsigned int i;
...
__weak bool
stm32mp1_ddr_interactive(void *priv, enum stm32mp1_ddr_interact_step step, const struct stm32mp1_ddr_config *config)
{
	return false;
}

#define INTERACTIVE(step) stm32mp1_ddr_interactive(priv, step, config)
...
static void wait_operating_mode(struct ddr_info *priv, int mode)
{
	...
	if (mode == DDRCTRL_STAT_OPERATING_MODE_SR) {
		...
		mask2 = DDRCTRL_STAT_OPERATING_MODE_MASK | DDRCTRL_STAT_SELFREF_TYPE_MASK;
		val2 = DDRCTRL_STAT_OPERATING_MODE_SR | DDRCTRL_STAT_SELFREF_TYPE_ASR;
	}
...
void stm32mp1_refresh_disable(struct stm32mp1_ddrctl *ctl)
{
	...
	clrbits_le32(&ctl->pwrctl, DDRCTRL_PWRCTL_POWERDOWN_EN | DDRCTRL_PWRCTL_SELFREF_EN);
...
ddr_start:
	...
	clrsetbits_le32(&priv->ctl->init0, DDRCTRL_INIT0_SKIP_DRAM_INIT_MASK, DDRCTRL_INIT0_SKIP_DRAM_INIT_NORMAL);
	 ...
	 */
	pir = DDRPHYC_PIR_DLLSRST | DDRPHYC_PIR_DLLLOCK | DDRPHYC_PIR_ZCAL | DDRPHYC_PIR_ITMSRST | DDRPHYC_PIR_DRAMINIT |
		  DDRPHYC_PIR_ICPC;
	...
	if (config->p_cal_present) {
		...
		stm32mp1_refresh_restore(priv->ctl, config->c_reg.rfshctl3, config->c_reg.pwrctl);
	} /* if (config->p_cal_present) */
