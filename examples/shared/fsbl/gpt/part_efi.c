...
static int part_test_efi(struct blk_desc *dev_desc)
{
	...
	if ((blk_dread(dev_desc, 0, 1, (ulong *)legacymbr) != 1) || (is_pmbr_valid(legacymbr) != 1)) {
		return -1;
	}
	return 0;
}
...
static int set_protective_mbr(struct blk_desc *dev_desc)
{
	...
	memset((char *)p_mbr + MSDOS_MBR_BOOT_CODE_SIZE, 0, sizeof(*p_mbr) - MSDOS_MBR_BOOT_CODE_SIZE);
...
int write_gpt_table(struct blk_desc *dev_desc, gpt_header *gpt_h, gpt_entry *gpt_e)
{
	const int pte_blk_cnt = BLOCK_CNT((gpt_h->num_partition_entries * sizeof(gpt_entry)), dev_desc);
	u32 calc_crc32;
	...
	if (blk_dwrite(dev_desc, le64_to_cpu(gpt_h->partition_entry_lba), pte_blk_cnt, gpt_e) != pte_blk_cnt)
		goto err;

	prepare_backup_gpt_header(gpt_h);

	if (blk_dwrite(dev_desc, (lbaint_t)le64_to_cpu(gpt_h->last_usable_lba) + 1, pte_blk_cnt, gpt_e) != pte_blk_cnt)
		goto err;

	if (blk_dwrite(dev_desc, (lbaint_t)le64_to_cpu(gpt_h->my_lba), 1, gpt_h) != 1)
		goto err;
...
int gpt_fill_pte(
	struct blk_desc *dev_desc, gpt_header *gpt_h, gpt_entry *gpt_e, disk_partition_t *partitions, int parts)
{
	lbaint_t offset = (lbaint_t)le64_to_cpu(gpt_h->first_usable_lba);
	lbaint_t last_usable_lba = (lbaint_t)le64_to_cpu(gpt_h->last_usable_lba);
	int i, k;
	...
	size_t pte_end = pte_start + gpt_h->num_partition_entries * gpt_h->sizeof_partition_entry / dev_desc->blksz;
	...
	for (i = 0; i < parts; i++) {
		...
		if (strlen(str_type_guid)) {
			if (uuid_str_to_bin(str_type_guid, bin_type_guid, UUID_STR_FORMAT_GUID)) {
				printf("Partition no. %d: invalid type guid: %s\n", i, str_type_guid);
				return -1;
			}
		} else {
			/* default partition type GUID */
			memcpy(bin_type_guid, &partition_basic_data_guid, 16);
		}
#else
		/* partition type GUID */
...
static uint32_t partition_entries_offset(struct blk_desc *dev_desc)
{
	 ...
	 */
	offset_bytes = PAD_TO_BLOCKSIZE(CONFIG_EFI_PARTITION_ENTRIES_OFF, dev_desc);
	 ...
	 */
	config_offset = fdtdec_get_config_int(gd->fdt_blob, "u-boot,efi-partition-entries-offset", -EINVAL);
...
int gpt_fill_header(struct blk_desc *dev_desc, gpt_header *gpt_h, char *str_guid, int parts_count)
{
...
int gpt_restore(struct blk_desc *dev_desc, char *str_disk_guid, disk_partition_t *partitions, int parts_count)
{
	gpt_header *gpt_h;
	gpt_entry *gpt_e;
	int ret, size;
	...
	size = PAD_TO_BLOCKSIZE(GPT_ENTRY_NUMBERS * sizeof(gpt_entry), dev_desc);
...
int gpt_verify_headers(struct blk_desc *dev_desc, gpt_header *gpt_head, gpt_entry **gpt_pte)
{
	/*
...
int gpt_verify_partitions(
	struct blk_desc *dev_desc, disk_partition_t *partitions, int parts, gpt_header *gpt_head, gpt_entry **gpt_pte)
{
	...
	for (i = 0; i < parts; i++) {
		...
		debug("%s: part: %2d name - GPT: %16s, ENV: %16s ", __func__, i, efi_str, partitions[i].name);

		if (strncmp(efi_str, (char *)partitions[i].name, sizeof(partitions->name))) {
			pr_err("Partition name: %s does not match %s!\n", efi_str, (char *)partitions[i].name);
			return -1;
		}

		/* Check if GPT and ENV sizes match */
		gpt_part_size = le64_to_cpu(gpt_e[i].ending_lba) - le64_to_cpu(gpt_e[i].starting_lba) + 1;
...
int is_valid_gpt_buf(struct blk_desc *dev_desc, void *buf)
{
	...
	if (validate_gpt_header(gpt_h, GPT_PRIMARY_PARTITION_TABLE_LBA, dev_desc->lba))
		return -1;

	/* determine start of GPT Entries in the buffer */
	gpt_e = buf + (le64_to_cpu(gpt_h->partition_entry_lba) * dev_desc->blksz);
...
int write_mbr_and_gpt_partitions(struct blk_desc *dev_desc, void *buf)
{
	...
	gpt_e = buf + (le64_to_cpu(gpt_h->partition_entry_lba) * dev_desc->blksz);
	gpt_e_blk_cnt =
		BLOCK_CNT((le32_to_cpu(gpt_h->num_partition_entries) * le32_to_cpu(gpt_h->sizeof_partition_entry)), dev_desc);
...
static int pmbr_part_valid(struct partition *part)
{
	if (part->sys_ind == EFI_PMBR_OSTYPE_EFI_GPT && get_unaligned_le32(&part->start_sect) == 1UL) {
		return 1;
	}
...
static int is_pte_valid(gpt_entry *pte)
{
	...
	if (memcmp(pte->partition_type_guid.b, unused_guid.b, sizeof(unused_guid.b)) == 0) {

		debug("%s: Found an unused PTE GUID at 0x%08X\n", __func__, (unsigned int)(uintptr_t)pte);
