...
{
#if CONFIG_IS_ENABLED(MAC_PARTITION) || CONFIG_IS_ENABLED(DOS_PARTITION) || CONFIG_IS_ENABLED(ISO_PARTITION) ||        \
	CONFIG_IS_ENABLED(AMIGA_PARTITION) || CONFIG_IS_ENABLED(EFI_PARTITION)
	puts("\nPartition Map for ");
	switch (dev_desc->if_type) {
		case IF_TYPE_IDE:
			puts("IDE");
...
int blk_get_device_by_str(const char *ifname, const char *dev_hwpart_str, struct blk_desc **dev_desc)
{
	char *ep;
	char *dup_str = NULL;
							...
							int allow_whole_dev)
{
	...
	if (!dev_part_str || !strlen(dev_part_str) || !strcmp(dev_part_str, "-"))
	 ...
	 */
	if (((*dev_desc)->part_type == PART_TYPE_UNKNOWN) || (part == 0)) {
		if (!(*dev_desc)->lba) {
										 ...
										 disk_partition_t *part_info)
{
	...
	if (!part_get_info_by_dev_and_name(dev_iface, dev_part_str, dev_desc, part_info))
		return 0;
	/*
	 * Couldn't lookup by name, try looking up the partition description
	 * directly.
	 */
	if (blk_get_device_part_str(dev_iface, dev_part_str, dev_desc, part_info, 1) < 0) {
