	...
	if (touched) {
		...
		if (ft6x06_TS_DetectTouch(0x70)) {
			if (H_RES > V_RES) {
				...
				ft6x06_TS_GetXY(
					0x70, reinterpret_cast<uint16_t *>(&data->point.y), reinterpret_cast<uint16_t *>(&data->point.x));
				data->point.y = 480 - data->point.y;
			} else {
				ft6x06_TS_GetXY(
					0x70, reinterpret_cast<uint16_t *>(&data->point.x), reinterpret_cast<uint16_t *>(&data->point.y));
			}
...
void MDMA_TransferComplete(MDMA_HandleTypeDef *han) { lv_disp_flush_ready(&disp_drv); }

void MDMA_Callback() { HAL_MDMA_IRQHandler(&hmdma); }
...
void DMA_Callback() { HAL_DMA_IRQHandler(&hdma); }
...
void TS_IO_Delay(uint32_t Delay) { HAL_Delay(Delay); }

void set_interrupt(uint32_t irq_num, uint32_t cpu, uint32_t priority, uint32_t edge_detect, void (*callback)())
{
	GIC_DisableIRQ(static_cast<IRQn_Type>(irq_num));
	GIC_SetTarget(static_cast<IRQn_Type>(irq_num), cpu);
	GIC_SetPriority(static_cast<IRQn_Type>(irq_num), priority);
	GIC_SetConfiguration(static_cast<IRQn_Type>(irq_num), edge_detect);
	irq_callback[irq_num] = callback;
	GIC_ClearPendingIRQ(static_cast<IRQn_Type>(irq_num));
	GIC_EnableIRQ(static_cast<IRQn_Type>(irq_num));
}
...
void __attribute__((interrupt("IRQ"))) IRQ_Handler()
{
	...
	GIC_EndInterrupt(static_cast<IRQn_Type>(irq_num));
}
}
